...
<body>
                                                ...
                                                <div class="list-group chat-list" id="chatList" style="max-height: 500px; overflow-y: auto;">
                                                    <ul class="list-group list-group-flush">
                                                        ...
                                                        <li class="list-group-item d-flex align-items-center chat-item">
                                                            <img src="{{ asset('uploads/groups/' . $chats->image) }}" class="profile_img rounded-circle mr-3" style="width: 40px; height: 40px;" alt="Profile Picture">
                                                                <div class="profile_info">
                                                                    <span class="profile_name font-weight-bold">{{$chats->name}}</span>
                                                                </div>
                                                            <span class="id" style="display: none;">{{$chats->id}}</span>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                                                         ...
                                                         <!-- If no chats found, show all users -->
                                                            @foreach ($users as $user)
                                                                <li class="list-group-item d-flex align-items-center">
                                                                    <img src="{{ asset('uploads/students/' . $user->image) }}" class="rounded-circle mr-3" style="width: 40px; height: 40px;" alt="Profile Picture">
                                                                    <div class="profile_info">
    ...
    <script>
        // Initialize Pusher
        var pusher = new Pusher('3f9fdbe05bb684beb746', {
            cluster: 'ap2',
            encrypted: true
        });

        // Subscribe to the channel
        var channel = pusher.subscribe('fetch-messages');

        // Bind to the event
        channel.bind('fetch-message', function(data) { // Updated event name to 'user-message'
            ...
            if (data && data.message) {
                ...
                if(data.user.id == {{ $LoggedAdminInfo->id }}){
                    /* let messageHtml = `
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.messages.forEach(function(message) {
                            ...
                            //let isSender = message.user_id == '{{ $LoggedAdminInfo->id }}';
                            /* let userAvatar = isSender ? '{{ asset('uploads/students/' . $LoggedAdminInfo->image) }}' : profileImage;
                            let userName = isSender ? '{{ $LoggedAdminInfo->name }}' : profileName; */
                            //let isSender = '{{ $LoggedAdminInfo->id }}';
                            
                            
                            //console.log({{ $LoggedAdminInfo->id }})
                            
                            //console.log(message.users[0].id)
                            //console.log(message.user_id);
                           // var image = asset('uploads/students/' ). message.users[0].image;
                           // console.log(image);
                           let userAvatar = `{{ asset('uploads/students/') }}/${message.users[0].image}`;


                            let userName = message.users[0].name;
                          
                            let messageTime = new Date(message.created_at).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit'
                            });
                            if({{ $LoggedAdminInfo->id }} == message.user_id){
                                let messageHtml = `
                                <div class="chat-message sender ">
                                    <div class="message-avatar">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            let userAvatar = '{{ asset('uploads/students/' . $LoggedAdminInfo->image) }}';
